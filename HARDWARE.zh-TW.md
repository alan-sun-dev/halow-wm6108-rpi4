...
## SG 的頻道規劃，以及唯一的那個 4 MHz 頻道

取自 AP 上的 `/usr/share/morse-regdb/channels.csv`——驅動自己查的那張表，不是回憶。
國碼 **SG**：

| 子頻段 | 頻寬 | s1g 頻道 | 中心 MHz | 映射 5 GHz | duty cycle | 最大發射 |
|---|---|---|---|---|---|---|
...
節點 1 跑的是 `channel 40`、`s1g_chanbw 4`、`country SG`——922.0 MHz，4 MHz 寬。

**SG 只有一個 4 MHz 頻道。** 第二台 AP 若用相同頻寬就必須同頻：2 MHz 的頻道 38
（920–922）與 42（922–924）都落在 4 MHz 頻道 40（920–924）**之內**，所以 920–925 這
個子頻段裡除了 1 MHz 的頻道 45 之外沒有不重疊的選擇。866–869 子頻段確實是分開的，但
duty cycle 被限制在 **2.77 %**，而且那正是規格錯誤的 HT-H7608 待過的地方。

對**對照組** AP 而言這不是限制而是正確的設計：同頻把 RF 環境固定住，於是一個共模事件
若打掉其中一台 AP 的 station 而沒打掉另一台，就把成因鎖定在 AP 而不是通道上——那正是
2026-09-18 那個發現所缺的判別器。

**在 morse phy 上讀 `iw` 的注意事項。** dot11ah shim 回報的永遠是映射後的 5 GHz 號
碼，不是真正的 S1G 頻率。節點 1 在 S1G 頻道 40、4 MHz 上，會顯示成一個 80 MHz 的 VHT
區塊，primary 是頻道 153（5765 MHz）、中心 5775 MHz——而 5775 就是映射頻道 155，也就
是表中 S1G 40 對應的 `map_5g_chan`。這個 repo 所有擷取裡的 `80MHz`、`VHT-MCS` 與
`freq 57xx` 都是這個 shim，不是無線電本身。
...
| `Allow-LuCI-mgmt`（2026-09-19 新增） | `wan` input | **TCP 443 連到 AP 自己**，來源含 `192.168.108.0/24` 與兩個管理網段。備份 `/etc/config/firewall.bak-20260919` |
...
`Allow-house-to-halow` 是 `proto all`，而這兩個新網段只開放 ICMP echo、TCP 22 與
TCP 443——其他一律丟棄，`iperf` 也包含在內。

**80 埠是刻意不開的。** `uhttpd` 同時監聽 `0.0.0.0:80` 與 `0.0.0.0:443`，只放行
443：LuCI 後面就是整個 HaLow 網段閘道的 root 密碼。這是用實際請求驗證的，不是看設定
檔——`https://192.168.108.5/` 回 **HTTP 200**，`http://` 沒有回應。要收回：
`uci delete firewall.cfg1492bd && uci commit firewall && /etc/init.d/firewall reload`。

### 2026-09-03 從兩個網段內部實測

筆電依序被移進 `192.168.200.0/24` 與 `192.168.101.0/24`，兩者行為完全相同。

| 目標 | ping | ssh |
|---|---|---|
| AP `192.168.108.5` | 0 % loss | 可以 |
| `dkmstest` `192.168.108.13` | 0 % loss | 可以 |
| station `10.41.0.208` | 100 % loss | 直連不行——**經 AP 跳板可以** |
| `dkmstest` `10.41.0.216` | 100 % loss | 同一個缺口 |

```
ssh -J root@192.168.108.5 alan@10.41.0.208     # 今天就能用，不需要動上游
```

AP 的 input 規則正在承載真實流量（counter 14 個封包）；兩條 `→ 10.41.0.0/16` 的
forward counter 仍是 0。三個儀器對「往 HaLow 的封包死在哪裡」的答案一致：counter 維持
0、AP `eth0` 上的 `tcpdump` 在同一時間窗看得到往 `192.168.108.5` 的對照 ping 卻看不到
任何一個往 `10.41.0.208` 的封包、而 `traceroute` 停在第 1 跳但同一台閘道在第 2 跳把往
`192.168.108.5` 的封包轉了出去。**封包從來沒有離開 VLAN 閘道，這件事與 AP 無關。**
...
VLAN 閘道**並不與** `192.168.108.0/24` 相鄰，所以裝在那裡的
`10.41.0.0/16 → 192.168.108.5` 下一跳是碰不到的：設定被接受，然後什麼也不做。這條路
由必須拆成兩段——VLAN 閘道把 `10.41.0.0/16` 送向它路徑上的那台 `192.168.0.0/24` 路由
器，而那一台持有 `10.41.0.0/16 → 192.168.108.5`。**兩個網段用的不是同一台**：`.200`
走 `192.168.0.1`，`.101` 走 `192.168.0.3`，所以兩台都要加。

判讀失敗的方式：從一個公網位址收到 `!X`，表示目的地漏到了 WAN 而被 ISP 路由器拒絕
——路徑上根本沒有那條路由。第 1 跳之後沉默，表示 VLAN 閘道把它吞掉了——路由存在但是
錯的。檢查方式是在有人從那兩個網段嘗試時，於 AP 上執行：

```
nft list ruleset | grep mgmt-subnets
```

counter 離開 0，表示封包已抵達 AP，剩下的問題在它的上游；counter 停在 0，表示它從來
沒有到達。
...
| 最長不中斷關聯 | **10 天 8 小時 28 分**，2026-09-05 13:16:20 → 2026-09-15 21:44:29，結束於一次**兩片板子在同一秒都看到的共模事件**，9 秒後恢復。先前的紀錄依序是 28 小時 34 分、123 小時 23 分 | **10 天 10 小時 47 分**，2026-09-05 10:56:47 → 2026-09-15 21:44:28——兩片板子產生過最長的關聯，跑在 6.12.96 上由 DKMS 安裝的模組。它「**開機至今未斷**」的那一段在 09-05 被同一類事件終結前達到 9 天 2 小時 2 分 |
