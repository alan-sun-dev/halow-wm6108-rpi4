...
| Other carrier pins | **GPIO 18** `halow-slot-power`, the M1's own mPCIe slot power enable, hogged high and also forced by `gpio=18=op,dh`. **GPIO 27** is held high by something on the HAT although the pin's own pull is down — undeclared, and the only unexplained line on the board (2026-09-18). Note that **GPIO 17 carries no consumer in debugfs even though it is RESET_N**: 2.0.1 drives it through the legacy integer GPIO API, so a debugfs-based survey reports it free | **GPIO 4** JTAG reset, hogged output-low (Heltec's `morse-ps.dtbo`) |
...
| Longest unbroken association | **10 d 8 h 28 m**, 2026-09-05 13:16:20 → 2026-09-15 21:44:29, ended by a **common-mode** event both boards saw in the same second and restored 9 s later. Earlier records: 28 h 34 m, then 123 h 23 m | **10 d 10 h 47 m**, 2026-09-05 10:56:47 → 2026-09-15 21:44:28 — the longest either board has produced, on the DKMS-installed module under 6.12.96. Its unbroken-**since-boot** run reached 9 d 2 h 2 m before the same class of event ended it on 09-05 |
...
| HAT button / fan / LED pins | 1–4 | **narrowed 2026-09-18, not closed.** The fan is on no GPIO the Pi drives and there is no tachometer input, so **its state cannot be read from software at all** — the fan is its own readout and the hunt needs a person at the box, whichever board it runs on. **GPIO 27 is the button lead**: an input whose own pull is *down* yet reads high, i.e. externally held up — the standard idle-high button wiring, and the only unexplained line on the board. Confirming it costs one press and one sample, and is safe on the soak node. Candidates after exclusions: 2, 3, 4, 6, 12, 13, 16, 19, 20, 21, 22, 25, 26 |
| cause of the common-mode disconnects | 1, 2, 5 | four events in the fifteen days to 2026-09-18 dropped **both** stations in the same second, at healthy RSSI, with the AP logging nothing before any of them. An AP transmitter that stops radiating silently and external interference on the channel look identical from here; separating them needs a spectrum capture or a third station in a third position |
