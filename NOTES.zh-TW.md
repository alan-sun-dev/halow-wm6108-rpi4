...
### 站台的吞吐量，第一次真正量到 —— 而且這條鏈路是 3 倍不對稱的

在站台上裝了 `iperf`（2.1.8+dfsg-1，跟 `dkmstest` 同一個版本，所以兩塊板子的方法完
全一致）。因為這塊板子正在 soak，先跑模擬：`0 upgraded, 1 newly installed, 0 to
remove and 208 not upgraded`，裝完 `dpkg -C` 是空的。那 208 個待升級的套件刻意沒有
動。

一跳 HaLow 打到 `10.41.254.1`，用 `ip route get` 確認過，每個方向 10 秒、TCP：

| | 站台 55:04（上面一層樓） | `dkmstest`（就在 AP 旁邊） |
|---|---|---|
| 上行，節點 → AP | **6.7–7.0 Mbit/s**（穩態半段 7.8/7.8、9.2/6.5） | 9.20 Mbit/s |
| 下行，AP → 節點 | **1.6–2.7 Mbit/s**，四次 | 8.60–8.81 Mbit/s |
| 比例 | **約 3 倍，上行佔優** | 對稱 |

每一次下行都是同一個形狀 —— 前半快、後半慢，然後拖著一條 5–6 秒、只搬 60–128 KB
的尾巴。四次都一樣，不是偶發。

**機制是量出來的，不是猜的。** 在一次下行測試的前後立刻讀 AP 側計數器：

```
AP -> 站台 55:04     tx_packets +2,616   tx_retries +621   tx_failed +261
                     每封包 0.24 次重送，10.0% 的訊框直接失敗
```

站台自己整段關聯期間的 `tx_failed` 是 3。失敗全部在 AP 的發送側，而那正是慢的那個
方向。

**讓這件事從「懷疑」變成「發現」的是對照組。** 同一個 AP、幾分鐘後、同樣的指令，對
著 `dkmstest`：

```
AP -> dkmstest       tx_packets +9,239   tx_retries +678   tx_failed +0
                     每封包 0.07 次重送，0.0% 失敗，8.81 Mbit/s
```

所以那 10% 的失敗率屬於站台這條路徑 —— 一層樓、−50 dBm —— 而**不是** AP 的發射
端。沒有這個對照，同一組數字會被讀成「AP 發送有問題」，而這張桌子已經為那個假設花
過一整天了。

**這一切都沒有傷到板子。** 大約 40 MB 的測試流量跑完，關聯一次都沒掉（12,192 秒還
在累加），`spi_errors 0`、`spi_timedout 0`、`dmesg` 失敗 0，事後 20 次 ping 回來
0% 遺失、平均 11.9 ms。一次把站台唯一的管理路徑灌滿 20 秒，代價是零。

**沒有確立的是：那些訊框為什麼會失敗。** −50 dBm 在紙上還有大約 40 dB 的餘裕，所以
單純的訊號強度解釋不了 10% 的失敗率。速率控制器在下行停在 MCS2–MCS3、上行卻是
MCS6，這跟失敗是一致的，但沒有說明成因。一層樓和一層木製天花板是最明顯的候選，而
兩者都沒有被量過。

**關於這組數字取代掉的那一組。** 早上那張表的站台欄（上行 3.56 / 下行 5.34）是往*另
一個*方向不對稱的，下行比上行快 —— 跟這裡六次執行的結果完全相反。這是不要使用它的
又一個理由；它的來源本來就不明。
