...
turned into a two-node 802.11s mesh. Exploratory, not project work — but it
produced measurements the project should keep, and it changed the lab's RF
environment in a way that will matter when the soak is next read.

### The factory defaults are not OpenWrt's, and two of them are traps

Read from `/rom` on the running AP rather than guessed:

| | value | where it comes from |
|---|---|---|
| LAN address | **`10.41.254.1/16`** | `/rom/etc/board.d/99-lan-ip`, unconditional — **not** 192.168.1.1 |
| LAN interface | **eth0** | `99-default_network`, the catch-all: neither `02_network` nor `03_openmanet_eth` matches `bcm2711,mm6108-spi` and neither has a `*)` arm, so the fallback fires |
| hostname / SSID | `BCM2711-<mac suffix>` | `00_system` via `morse_generate_default_hostname` — automatically unique per board |
| root password | **none** | see below |

**Trap 1: the Ethernet port is a DHCP server.** `eth0` sits inside `br-lan` and
dnsmasq serves `lan`, so a freshly flashed box plugged into the house switch
hands out `10.41.0.100–249` to everything on it. The lease file was empty when
checked, so nothing took one. This is the second time the project has recorded
this warning — the first was 2026-08-23.

**Trap 2: `country=US`, `channel=42`, no `s1g_chanbw` (so 2 MHz), txpower
27 dBm**, against the lab's SG / 40 / 4 MHz / 22 dBm. Both regdomains define a
4 MHz channel 40 at **922.0 MHz** — the same frequency — but the op class
differs (3 against 21) and the power cap differs a lot: **36 dBm for US against
22.15 for SG**. US's other 4 MHz channels are 906, 910, 914, 918 and 926 MHz,
all outside the 920–925 band this lab operates in.

### Why root has no password, traced to its cause

`/rom/etc/uci-defaults/31_password` reads a `device_password` from persistent
storage and runs `passwd root` with it. The command it uses,
`persistent_vars_storage.sh`, **does not exist anywhere in the image** — and the
script's own comment says its absence "is expected" and produces no output. So
the variable is empty, `passwd` is never called, and **a blank root password is
the shipped state of this image**, not something anyone forgot to set. It
explains both boxes, and the `Auth succeeded with blank password for 'root'`
lines in the AP's log.

The Wi-Fi PSK, by contrast, is per-device: generated by `openmanetd` and stored
as `option default_wifi_key` in `/etc/config/system`. Two boxes' keys hash
differently, so it is not a shared factory constant. `/lib/wifi/morse.sh` does
carry a hardcoded fallback — `ssid=MorseMicro`, `key=12345678` — but neither
board is using it.

### Mesh Point against Mesh Gate, in the wizard's own words

> **Mesh Gate**: "a Mesh Point with a co-located non-mesh network (e.g. an AP,
> an upstream Ethernet connection, etc.). It broadcasts mesh gate announcements
> to help align the mesh nodes, making it easier for traffic to reach the
> non-mesh network."

Point is a member; Gate is the exit. One or two Gates per mesh, the rest Points.

Worth noting for anyone reading this later: **the box is already an AP out of
the box**, so neither wizard is needed to get an AP. The mesh wizard exists to
turn it into something else.

**Choosing Gate also removes trap 1.** The wizard sets the uplink to
`proto dhcp` — eth0 becomes a DHCP *client* — and moves dnsmasq to the mesh
side. The generated config then carries `no-dhcp-interface=eth0` explicitly, and
the box is safe to plug into the house switch. That was confirmed after the fact:
eth0 took `192.168.108.23` from the house router and served nothing.

### Changing one end of a mesh is changing half of it

The wizard put `br-ahwlan` on **`10.41.0.1/16`** — the same /16 as the lab's
HaLow segment, whose gateway is `10.41.254.1` and whose DHCP pool is
`10.41.0.100–249`. The new box's pool was `10.41.0.100–115`: a direct overlap
between two separate networks, plus a UniFi static route sending all of
`10.41.0.0/16` to node 1.

The gate was moved to `10.42.0.1/16`. dnsmasq followed automatically
(`dhcp-range=set:ahwlan,10.42.0.100,10.42.0.115`), and the eth0 session used to
make the change was untouched because `reload` only applies interfaces that
changed.

**That broke IPv4 to the second node**, which the wizard had placed at
`10.41.254.103/16`. The mesh itself was fine — L2 peering and IPv6 link-local
both worked throughout — but the two ends were on different subnets, which is
how a `10.42.0.112 FAILED` entry appeared in the gate's neighbour table. Fixed
by moving the second node to `10.42.254.103`, reached over IPv6 link-local
through the mesh. **The mesh plink stayed `ESTAB` across the whole
reconfiguration.**

The lesson is small and general: an address change on a mesh gate is not a local
change.

### The mesh, measured

Two nodes, batman-adv `BATMAN_V`, plink `ESTAB`. The second node's `eth0` is
**DOWN** — everything reaching it goes over 900 MHz.

```
latency     batctl ping x20    2.626 / 3.184 / 5.175 ms, 0% loss
throughput  batctl tp          9,638,280 bytes in 10.69 s = 7.21 Mbit/s
```

Two things worth keeping from that:

**7.21 Mbit/s is this hardware's ceiling, reached by a different path.** The
project's best previous figure is 7.4 Mbit/s, measured with iperf over AP/STA at
4 MHz with a 50 MHz SPI clock. This is batman-adv's own meter over an 802.11s
mesh — a completely different software path — landing within 3%. Two independent
routes to the same number say it is the radio and the air interface, not a
software layer. **The mesh costs almost nothing.**

**batman-adv's own estimate was 14.9 Mbit/s, about twice the measured value.**
It joins `ap_expected_thr` and idle MCS on the list of numbers this radio reports
that cannot be used as measurements. It is an input to route selection, not a
throughput figure.

The 3.18 ms latency is not evidence that mesh beats AP/STA: these two boards are
beside each other at −19 dBm on a single hop, against the soak station's 6–141 ms
over distance.

### What this changes for the soak, and it is not nothing

The new mesh is on **922.0 MHz at 4 MHz under SG — the same channel as node 1** —
and a scan from the mesh sees node 1's SSID at **−14 dBm**. From today the lab's
measurement channel has a co-channel neighbour, and it is not a quiet one:
two nodes running batman-adv's periodic broadcasts.

The throughput test was run with before-and-after snapshots of node 1, because
saturating a shared channel for ten seconds is exactly the sort of thing that
could drop a station. It did not: both associations advanced by 11 seconds with
no reset.

But the standing open question from 2026-09-18 — four common-mode disconnects in
fifteen days, cause unknown, AP silent before each — **now has a new candidate
cause that did not exist before 2026-09-19.** Any change in the pattern from here
must be read with that in mind. It also cuts the other way: a co-channel
neighbour under our control is an experiment we could not previously run.

### A mesh point's Ethernet port is a feature, and two DHCP servers were not

Asked whether a laptop plugged into the mesh point's `eth0` would reach the
internet. It would, and the node already does:

```
br-ahwlan members       bat0, eth0, phy1-ap0
default route           via 10.42.0.1 dev br-ahwlan
ping 1.1.1.1            0% loss, 23-45 ms   (laptop -> mesh -> gate -> house -> internet)
DNS                     resolves
```

So the DHCP server on a mesh point's Ethernet is the **intended** behaviour for
this product: carry the node into the field, plug a laptop in, and the traffic
leaves over HaLow. The earlier framing of it as a hazard was one-sided — the
hazard is only plugging that port into a network that already has DHCP.

**The actual fault was worse and was already live.** Both boxes were serving
`dhcp-range=10.42.0.100,10.42.0.115` — the *identical* pool — and batman-adv
bridges both `br-ahwlan` interfaces into **one L2**. Two DHCP servers racing on
one segment with the same range, which is how two clients get the same address.
Not a latent risk: it had been true since the second node joined.

Fixed the standard way for a bridged mesh — one server, on the gate:
`dhcp.ahwlan.ignore=1` on the mesh point. Verified on both sides: the point's
generated config now carries `no-dhcp-interface=br-ahwlan` with no `dhcp-range`
at all, the gate still serves, and the point's ARP entry for `10.42.0.1` proves
broadcasts cross the mesh — so a laptop on the point's `eth0` still gets its
lease, from the gate. Backup `/etc/config/dhcp.bak-20260919`.

**Instrument note, the third of the day.** The fix was first proposed as
`uci set dhcp.lan.ignore=1`, copied from the gate's layout. **`dhcp.lan` does
not exist on the mesh point**; the serving section is `dhcp.ahwlan`. That
command would have returned success and changed nothing. Together with the
dropbear path and the invalid key check, three failures today from assuming a
name instead of reading it on the machine in front of me.
...
| mesh | `Sun` / ch 40 / SG / SAE | identical |
| eth0 | `192.168.108.23`, DHCP client | DOWN |

Reachable as `ssh -J root@192.168.108.23 root@10.42.254.103`. Both now carry the
laptop's key and a root password, which took one lockout to get right — see
below.

### Two lessons from closing the blank root password, and the second is mine

**`dropbear` on OpenWrt reads `/etc/dropbear/authorized_keys`, not
`/root/.ssh/authorized_keys`.** That is a departure from OpenSSH that this
project had not recorded. The key was installed in the wrong place on both
boxes.

**A blank root password hides that completely.** With no password set, dropbear
admits the session whatever the client offers, so a key in the wrong file still
appears to work. The failure surfaces only once a password exists — which is
precisely when the fallback disappears. Both boxes locked out at that moment and
had to be recovered with the password, interactively.

**And the verification that was supposed to catch this could not have.** The
check run before the password was set was
`ssh -o PreferredAuthentications=publickey … 'echo KEY-AUTH-OK'`, and it
returned OK. It proved nothing: on a passwordless account that command succeeds
whether or not the key is readable, so it **gives the same answer in the broken
state and the working state.** This file has recorded that rule since
2026-08-26 — *a control must be checked in the state where it should fail* — and
it was broken here by the person writing it down.

The correct order is therefore stricter than "key first, password second":

```
1. install the key in /etc/dropbear/authorized_keys
2. set the root password
3. THEN verify key-only login — before step 2 the test is meaningless
```

Re-verified after recovery, with the password in place so the test is real:
publickey-only login returns OK on both boxes, `/etc/dropbear/authorized_keys`
is 600 and 93 bytes on each, root's shadow field is a 63-character hash, and the
mesh plink stayed `ESTAB` throughout.

## 2026-09-19 — what it actually costs to run this board as the AP, and the version question answered from the shelf

Asked which OpenWrt to use for a second HaLow AP, and then what the problems
with running one are. Both are answerable from node 1 rather than from memory:
it has been the AP for this segment for 23 days. Everything below was read off
the running system today.

### The problems with running this board as the AP

Consolidated here because it has been scattered across eight months of entries,
and because a second AP is being planned. None of it is generic OpenWrt advice —
every item was measured on this bench.

**1. It drops both clients at once, and nothing knows why.** Four times in the
fifteen days to 09-18, both stations disconnected in the same second and
recovered in about ten seconds. The AP logged *nothing* in the ten minutes
before any of them. Unresolved: a transmitter that stops radiating silently and
channel interference are indistinguishable from the two positions we have.
Ten-second outages are survivable for a console server; **an unexplained cause
is not, because nothing bounds it at ten seconds.**

**2. There is a stall that only one recovery step fixes.** The ladder, from the
2026-08-23 work: `wifi reload` ✗ → debugfs `restart` ✗ → debugfs **`reset` ✓** →
reboot ✓. So there is a failure mode where the one thing that works requires
already being able to log in to the AP.

**3. It is a single point of failure, and losing it loses the way back in.**
Both stations default out over HaLow, so the AP is their only exit. The A1
station has no house-LAN address at all — `wlan0` dormant, `eth0` down. If the
AP goes, both clients are unreachable by every remote path simultaneously and
only physical access remains. The 2026-08-27 review named this the largest
deployment risk; it is still open.

**4. Low bandwidth, and in SG no second channel to move to.** Measured
1.3–1.5 Mbit/s down and 0.2–0.9 up at 2 MHz, about 7.4 Mbit/s once the SPI clock
went to 50 MHz with 4 MHz of air. Fine for a text console, painful for anything
bulk. And per the regdb, **SG has exactly one 4 MHz channel** — 922.0 MHz. An
interferer there cannot be avoided by moving; the 1 and 2 MHz channels sit
inside the same span. See `HARDWARE.md`, "The SG channel plan".

**5. Several of the numbers it reports cannot be trusted.** RSSI saturates, so
it does not track distance or quality. `ap_expected_thr` and idle MCS were shown
to move 5.6x with no effect on the real retry cost. A ghost station entry looks
exactly like a working client. And the AP logs in UTC while both Pis log local
time, +8 — an easy way to misread a timeline.

**Three configuration traps that bite on a fresh flash**, all currently set
correctly on node 1 and all easy to reintroduce:

| trap | what it looks like |
|---|---|
| country mismatch | the two ends simply cannot see each other, scans return empty, and nothing says why (US vs SG, 2026-08-23) |
| power save on the station | inbound traffic vanishes while outbound is fine — looks like packet loss, is not |
| `openmanetd` left running | bulk transfer dies in **one direction** while ping still works: the MTU black hole |

**And one that belongs to the image, not the board:** the OpenMANET image ships
with **no root password and password authentication enabled**, so a freshly
flashed node grants root to anything that can reach port 22. That is a property
of the vendor image, verifiable by flashing it, and closing it is the first step
on any node that will be deployed rather than sat on a bench.

Stated fairly: as an AP the board does its job — 23 days, `errors 0`, WPA3-SAE
with PMF, DHCP, NAT. **The problem is not whether it can be an AP. It is that it
occasionally fails, the cause is unknown, and there is no second way in to fix
it afterwards** — which is why unattended recovery outranks adding a second AP.

### The version question: OpenMANET 1.8.0, which node 1 already runs

Releases are at `OpenMANET/firmware`, not `OpenMANET/openwrt` (that repo has
none). Latest is **1.8.0, 2026-08-16** — and node 1 reports `OpenMANET 24.10
1.8.0`, so the reference AP is already current.
...
Three fields must be right: `rpi4`, `mm6108`, **`spi`**. Its board profile is
`RPI RPI4-MM6108 (SPI)` / `bcm2711,mm6108-spi` — the M1 is a first-class target
in this image, which is the reason to prefer it. It carries the SPI-core patch
that makes Morse's init sequence work, which is why OpenMANET never hit the
three defects this project had to fix on Raspberry Pi OS.

1.8.0's own changes are almost entirely `openmanetd` — MAVLink, CoT, RTP, audio,
WebUI. **For a plain AP none of the headline features apply**, which is another
argument for leaving that daemon stopped.

### A naming trap worth adding to the 2026-08-29 list

The `spi` image ships exactly one Morse module and it is called
**`/lib/modules/6.6.138/mm6108_sdio.ko`**. It registers the `morse_spi` driver,
and `/sys/bus/spi/devices/spi0.0/driver` resolves to `.../drivers/morse_spi`.
So on a board with no SDIO HaLow anywhere, `lsmod` shows `mm6108_sdio`. The
2026-08-29 entry lists three sources of the false "this lab runs SDIO"
impression; **this is a fourth, and it is the most misleading, because it sits
on the SPI AP itself.** Check the bus binding, never the module name.

(For the same reason, an earlier statement in this session that the driver is
"built into the kernel" was wrong — it is a loadable module. The grep behind it
filtered on `morse`, which the module name does not contain.)

### Seeed's recommended fork does run on a Pi 4, and how it solves the SPI problem

`Wvirgil123/openwrt`, branch `seeed`, one release: v2.7-dev (2025-01-09), last
push 2025-08-13. Its single image is `morse-micro-ekh01`, and in
`target/linux/bcm27xx/image/Makefile`:

```make
define Device/morse_ekh01
  $(Device/rpi-4)          # inherits the Pi 4 device wholesale
...
So it **is** a Pi 4 image — `kernel8.img`, the `bcm2711-rpi-4-b` DTB, bcm2711
only. Note it overrides `SUPPORTED_DEVICES` and drops `raspberrypi,4-model-b`:
irrelevant to writing an SD card, relevant to `sysupgrade`, which checks it.

How it gets past the chip-select problem is the interesting part:
`999-001-morse-spi-fix-spi-bcm2835-driver-v5.3.patch`, **1139 lines**, reverts
`spi-bcm2835.c` to a v5.3-era implementation and swaps the gpio descriptor API
back to `of_gpio.h`. No descriptors means `spi_setup()` never forces
`SPI_CS_HIGH`, so defect 2 cannot arise — but it gets there by regressing the
whole controller driver rather than by Morse's later, surgical
`991-0007` flag. Everything that landed in `spi-bcm2835` between 5.3 and 5.15
goes with it.

Against OpenMANET 1.8.0 it is kernel 5.15 vs 6.6.138, driver 2.7-dev vs 2.0.x,
and no M1 board profile — the overlay would need swapping to the WM1302 pinout
and the BCF corrected to `bcf_fgh100mhaamd.bin`. That last one is not a
footnote: the HC01P shipped pointing at EKH01-03's `bcf_mf08551.bin` and it cost
that module its transmitter until 2026-08-24. Not run here; this is all read
from the tree.

### A second AP is the right instrument for the 09-18 question

Not a third station. Three stations would still all be listening to one AP, so a
common-mode event stays ambiguous. Two APs resolve it: an event that drops one
AP's clients and not the other's isolates the AP; one that drops both is the
channel or the environment.

Because SG has only the one 4 MHz channel the second AP must be **co-channel** —
and for this experiment that is correct rather than a compromise, since it holds
the RF environment constant.

Four conditions, or it is not a control: it needs **its own station**; a
**different SSID** (same SSID lets a station roam and the assignment is lost);
**NTP on both** (the whole cross-match method is second-level); and the existing
stations must **not** be moved, since the A1/A2 pair on node 1 is the instrument
that produced the 09-02 and 09-18 findings.

The phy supports IBSS, managed, AP, AP/VLAN, **monitor** and mesh point, with
`#{managed, AP, mesh point} <= 2, total <= 2, #channels <= 1`. Two consequences:
one board could be both AP-2 and a third station, but sharing one radio makes a
radio fault indistinguishable from a channel event, which destroys it as a
control; and **monitor mode may be the cheap substitute for the spectrum capture
the TBD row asks for** — a board parked on the channel recording beacons would
show directly whether AP-1's beacons stop while the channel stays quiet.
Untested: the phy advertising monitor and the driver implementing it usefully
are different claims, and this project has been caught by that gap before.

### LuCI

OpenMANET ships a full LuCI plus `luci-app-morseconfig` (HaLow settings as a web
form), `luci-app-ekhwizards`, `luci-app-ttyd` (a browser terminal),
`luci-app-statistics`, and an OpenMANET theme. `uhttpd` listens on `0.0.0.0:80`
and `:443`, but the `wan` zone rejected both until today.

`Allow-LuCI-mgmt` now permits **TCP 443 only** from `192.168.108.0/24` and the
two management subnets; port 80 was left closed deliberately. Verified by
request rather than by config: `https://192.168.108.5/` returns HTTP 200 and
`http://` returns nothing. Backup `/etc/config/firewall.bak-20260919`. Details
in `HARDWARE.md`.
